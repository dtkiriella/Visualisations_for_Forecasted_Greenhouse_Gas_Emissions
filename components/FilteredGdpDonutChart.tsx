...
const COLORS = [
  "#ff0080", "#ff6b00", "#00f5ff", "#00ff88", "#ffff00",
  "#ff0040", "#9d00ff", "#00ffc8", "#ff8800", "#0080ff"
];

export default function FilteredGdpDonutChart() {
    ...
    const loadCountries = async () => {
      try {
        ...
        if (json.countries && json.countries.length > 0) {
          setSelectedCountries(["USA", "CHN", "JPN", "DEU", "IND"].filter(code => 
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setLoadingData(true);
      try {
        const res = await fetch(`/api/data/compare?type=gdp&countries=${selectedCountries.join(",")}`);
        const json = await res.json();
        setData(json.data ?? []);
      } catch (e) {
        console.error(e);
    ...
    <div className="rounded-3xl bg-slate-900/70 p-6">
      <div className="mb-4">
        <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
          GDP by Year - Selected Countries
        </h3>
        <p className="mt-1 text-[10px] text-slate-500">
          Historical GDP data filtered by country
        </p>
      </div>
                ...
                {filteredCountries.map((country) => (
                  ...
                  >
                    <input
                      ...
                      className="h-3 w-3 rounded border-slate-600 bg-slate-700 text-purple-500 focus:ring-2 focus:ring-purple-500"
                      onClick={(e) => e.stopPropagation()}
                    />
