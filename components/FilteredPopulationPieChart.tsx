...
const COLORS = [
  "#00f5ff", "#ff0080", "#ffff00", "#00ff88", "#ff6b00",
  "#0080ff", "#ff0040", "#9d00ff", "#00ffc8", "#ff8800"
];

export default function FilteredPopulationPieChart() {
    ...
    const loadCountries = async () => {
      try {
        ...
        if (json.countries && json.countries.length > 0) {
          setSelectedCountries(["CHN", "IND", "USA", "IDN", "PAK"].filter(code => 
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setLoadingData(true);
      try {
        const res = await fetch(`/api/data/compare?type=population&countries=${selectedCountries.join(",")}`);
        const json = await res.json();
        setData(json.data ?? []);
      } catch (e) {
        console.error(e);
    ...
    <div className="rounded-3xl bg-slate-900/70 p-6">
      <div className="mb-4">
        <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
          Population by Year - Selected Countries
        </h3>
        <p className="mt-1 text-[10px] text-slate-500">
          Historical population data filtered by country
        </p>
      </div>
                ...
                {filteredCountries.map((country) => (
                  ...
                  >
                    <input
                      ...
                      className="h-3 w-3 rounded border-slate-600 bg-slate-700 text-cyan-500 focus:ring-2 focus:ring-cyan-500"
                      onClick={(e) => e.stopPropagation()}
                    />
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {selectedCountries.map((code) => {
              ...
              return (
                <span
                  key={code}
                  className="inline-flex items-center gap-1 rounded-full bg-cyan-500/20 px-2 py-1 text-xs text-cyan-300"
                >
                  {country?.name || code}
                  <button
